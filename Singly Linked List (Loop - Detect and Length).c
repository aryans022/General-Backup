...
int lengthLoop(struct node *head) {                                         //checks if a loop is present in the linked list
     ...
     while (slow != NULL && fast != NULL && fast->next != NULL) {          //while all can travel
          ...
          if (slow == fast && length == 0) {                               //slow and fast will meet if there is a loop
               flag = 1;
          }

          if (slow == fast  && length != 0) {
               return length;
          }
